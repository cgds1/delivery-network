...
async function registrarEnvio() {
  ...
  const sender_phone = readlineSync.question('Telefono remitente: ');
  ...
  const weight_kg = readlineSync.questionFloat('Peso (kg): ');
...
async function buscarPorCodigo() {
  ...
  try {
    ...
    console.log(`Direccion:    ${detail.shipment.receiver_address}`);
    console.log(`Descripcion:  ${detail.shipment.description}`);
    console.log(`Peso:         ${detail.shipment.weight_kg} kg`);
    console.log(`Estado:       ${statusColor(detail.shipment.status)}`);
    console.log(`Fecha:        ${formatDate(detail.shipment.created_at)}`);
  } catch (e) {
    console.error('Error:', e.message);
  }
  pause();
